 ...
 * for all benchmarks from Yahoo Finance and upserts into mp_benchmark_prices.
 *
...
import { getHistoricalPrices, toYahooSymbol } from "@/lib/yahoo-finance";
...
export async function POST() {
  ...
  for (const bench of benchmarks) {
    try {
      const yahooSymbol = toYahooSymbol(bench.ticker);
      const bars = await getHistoricalPrices(yahooSymbol, fromDate, toDate);

      if (!bars.length) {
        results.push({ benchmark: bench.name, inserted: 0, error: "No data from Yahoo Finance" });
        continue;
      }
